function [plots,rssi1,rssi2]=pluto_LTE(ip,LTEmode,plots)
...
txsim.DesiredCenterFrequency = 2.45e9+5e6; % Center frequency in Hz
...
tx.uri = ip;
rx.uri = ip;
rx.SamplesPerFrame = 2^20;%length(eNodeBOutput)*3;
rx.EnableCustomFilter = true;
rx.CustomFilterFileName = fir_data_file;
tx.EnableCustomFilter = true;
tx.CustomFilterFileName = fir_data_file;
rx.CenterFrequency = txsim.DesiredCenterFrequency;
tx.CenterFrequency = txsim.DesiredCenterFrequency;

rssi1 = 0;
rssi2 = 0;

fprintf('Starting hardware loopback capture\n');
...
fprintf('Starting LTE decode\n');
clear plots; close all;
