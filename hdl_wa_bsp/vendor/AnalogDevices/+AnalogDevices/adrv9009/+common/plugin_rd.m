...
hRD.ReferenceDesignName = sprintf('ADRV9009 %s Base System (Vivado 2017.4.1)', upper(board));

% Determine the board name based on the design
hRD.BoardName = sprintf('AnalogDevices ADRV9009 %s (%s)', upper(board), design);

% Tool information
hRD.SupportedToolVersion = {'2017.4.1'};
...
% 			'CustomTopLevelHDL',    fullfile('projects', 'adrv9009', lower(board), 'system_top.v'));
	case 'OBS'
		hRD.addCustomVivadoDesign( ...
			'CustomBlockDesignTcl', fullfile('projects', 'adrv9009', lower(board), 'system_project_rx_obs.tcl'), ...
			'CustomTopLevelHDL',    fullfile('projects', 'adrv9009', lower(board), 'system_top.v'));
    otherwise
		hRD.addCustomVivadoDesign( ...
